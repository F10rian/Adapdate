...
def update_adapter(arch="adapter-net", checkpoint_path="./", checkpoint_adap_path="./", v=3, R=2):
    ...
    if arch == "adapter-net":
        ...
        small_model = Adapter_for_adapter_net(v=v, R=R)
        small_model.load_state_dict(torch.load(checkpoint_adap_path))
        small_model.eval()
...
def parse_args(argv):
    ...
    parser.add_argument("--adapter-path", type=str, help="Path of the adapter.")
...
def main(argv):
    ...
    update_adapter(arch=args.model, checkpoint_path=args.model_path, checkpoint_adap_path=args.adapter_path, v=args.v, R=args.R)
